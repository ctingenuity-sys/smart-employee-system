...
const endIndex = lines.findIndex(l => l.includes('<div className="max-w-5xl mx-auto px-4 py-8 animate-fade-in" dir={dir}>'));
...
const middle = `
    ...
    const handlePrintLeave = async (leave: LeaveRequest) => {
        await handlePrintItem({ ...leave, titleEn: 'LEAVE REQUEST', titleAr: 'طلب إجازة', type: 'leave' });
    };

    const handlePrintSwap = async (swap: SwapRequest) => {
        await handlePrintItem({ ...swap, titleEn: 'SWAP REQUEST', titleAr: 'طلب تبديل', type: 'swap' });
    };
    ...
    const handlePrintItem = async (item: any) => {
        try {
            ...
            const uDoc = await getDoc(doc(db, 'users', fullData.from || fullData.userId || currentUserId));
            ...
            if (fullData.supervisorApproval) {
                ...
                supApproved = false;
            } else if (fullData.status === 'approvedBySupervisor') {
                supApproved = true;
            }
...
console.log('Successfully replaced middle.');
