...
const ModalityLogbook: React.FC<ModalityLogbookProps> = ({ type, title, colorTheme = 'slate' }) => {
    ...
    const [loading, setLoading] = useState(false); // Changed default to false (lazy load)
    ...
    const fetchLogs = async () => {
        ...
        try {
            ...
            const constraints: any[] = [
                where('status', '==', 'done'),
                where('completedAt', '>=', queryStart),
                where('completedAt', '<=', queryEnd)
            ];

            // Add Modality Filter to Query
            if (type === 'X-RAY') {
                // Fetch X-RAY and OTHER together if possible, or just strict X-RAY
                // Using 'in' operator to save reads
                constraints.push(where('examType', 'in', ['X-RAY', 'XRAY', 'OTHER']));
            } else {
                constraints.push(where('examType', '==', type));
            }
            ...
            console.error(e);
            let msg = 'Error fetching logs';
            if (e.message?.includes('index')) {
                msg = 'Missing Index. Please ask Admin to click the link in console.';
            }
        ...
        reader.onload = (event) => {
            try {
                ...
                if (Array.isArray(json)) {
                    ...
                    setToast({ msg: `Imported ${filtered.length} records locally`, type: 'success' });
                } else {
                    setToast({ msg: 'Invalid File', type: 'error' });
                }
            } catch (err) {
    ...
    const handleUpdateCounter = async () => {
        ...
        try {
            ...
            setToast({msg: 'Counter updated successfully', type: 'success'});
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20 font-sans print:bg-white print:p-0" dir={dir}>
            ...
            <PrintHeader title={`${title} Logbook`} subtitle={isArchiveMode ? "Archived Data (Local)" : `${startDate} to ${endDate}`} themeColor={colorTheme} />
            ...
            <div className="max-w-7xl mx-auto px-4 py-8 print:p-0 print:max-w-none">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 print:hidden">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 font-bold">Completed Cases Register</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 bg-white p-2 rounded-2xl shadow-sm border border-slate-200">
                        {isArchiveMode ? (
                            <div className="px-3 py-1 bg-amber-100 text-amber-800 rounded-lg text-xs font-bold border border-amber-200 flex items-center gap-2">
                                <i className="fas fa-history"></i> Archive View
                                <button onClick={fetchLogs} className="text-red-500 hover:text-red-700 underline ml-2">Back to Live</button>
                            </div>
                        ) : (
                            <>
                                ...
                                <button onClick={fetchLogs} className={`w-9 h-9 rounded-lg text-white flex items-center justify-center hover:opacity-90 transition-all shadow-md ${activeBgClass}`} title="Search / Refresh">
                        ...
                        <div className="relative">
                            ...
                            <button className="bg-slate-100 text-slate-600 px-3 py-2 rounded-lg text-xs font-bold hover:bg-slate-200 flex items-center gap-2 border border-slate-200">
                                <i className="fas fa-file-upload"></i> Archive
                            </button>
                        </div>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden print:shadow-none print:border-2 print:border-black print:rounded-none">
                            ...
                            <table className="w-full text-sm text-right print:text-xs">
                                <thead className={`text-white font-bold uppercase ${activeBgClass} print:text-black print:bg-slate-200 print:border-b-2 print:border-black`}>
                                    <tr>
                                        ...
                                        <th className="p-4 print:p-2">Completed At</th>
                                        <th className="p-4 print:p-2">Patient Name</th>
                                        <th className="p-4 print:p-2">File No (ID)</th>
                                        <th className="p-4 print:p-2">Exam</th>
                                        <th className="p-4 print:p-2">By</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 print:divide-slate-300">
                                    {logs.length === 0 ? (
                                        <tr><td colSpan={7} className="p-8 text-center text-slate-400">No records found. Click search button to load.</td></tr>
                                    ) : (
                                        logs.map((row, i) => (
                    ...
                    <div className="p-4 bg-slate-50 border-t border-slate-200 print:bg-white print:border-t-2 print:border-black flex justify-between items-center font-bold text-xs">
                        <span>Total Cases: {logs.length}</span>
            ...
            {/* Settings Modal */}
            <Modal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} title="Numbering Settings">
                <div className="space-y-6 text-center">
                    <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 text-amber-900 text-sm">
                        <i className="fas fa-exclamation-triangle mb-2 text-2xl"></i>
                        <p className="font-bold">Important</p>
                        <p>Changing this number affects the NEXT patient. Be careful to avoid duplicates.</p>
                    </div>

                    <div className="flex flex-col items-center gap-2">
                        <span className="text-xs font-bold text-slate-400 uppercase">Current Counter</span>
                    ...
                    <div className="text-right">
                        <label className="text-xs font-bold text-slate-500 mb-1 block">Set Next Number To:</label>
                        <input 
                            type="number" 
                        ...
                        />
                        <p className="text-[10px] text-slate-400 mt-1">Next Patient will be: {parseInt(newCounter || '0')}</p>
                    </div>
